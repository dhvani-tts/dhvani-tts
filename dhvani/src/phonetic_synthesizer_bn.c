...
void bn_readfile();
char *bn_process(short *word, int size);
int bn_replace(unsigned short *s, int size);
char *bn_replacenum(unsigned short *s, int size);
char *bn_parsenum(char *numstr);
char *bn_spellNumbers(short *word, int start, int end);
char *bn_parseMiscellaneous(short *s, int size);
...
char *bn_available_halfs[] ={"ky", "kr", "kl", "kll", "kv", "ksh", "khy", "khr", "khl", "khv",
    "gy",
...
int
bn_replace(unsigned short *s, int size) {
    int i, j;
    ...
    while (i < size) {
        switch (s[i]) {
            ...
            case 0x099E: /*nja */
                if (i <= size - 1 && s[i + 1 == 0xD4D] && s[i + 1 == 0xD1E]) {
                    i += 2;
            ...
            case 0x09B1: /*R */
                if (i + 2 <= size && s[i + 2] == 0x09B1 && s[i + 1] == 0x09CD) {
                    // handling rra-> tra    
                    word[j].letter = "t";
                    word[j].type = 1;
                    j++;
...
char *
generate_phonetic_script_bn(short *word, int size) {
    ...
    if ((word[0] >= 0x09E6 && word[0] <= 0x09EF)
        ...
        if (i == size || i == size - 1) //No decimal points
        {
            if (size > 9) {
                ...
                final = strcat(final, bn_spellNumbers(word, 0, size));
            } else { //construct the number string using bn_parsenum logic
                //printf("reading numbers\n");
                final = strcat(final, bn_parsenum(bn_replacenum(word, size)));
            }
        } else {
            ...
            if (i > 9) {
                ...
                //Read each numbers seperately  
                //      printf("spell out numbers\n");
                final = strcat(final, bn_spellNumbers(word, 0, i));
                //      printf("spell out numbers-over\n");
            } else { //construct the number string using bn_parsenum logic
                //      printf("speak numbers%d\n",i);
                final = strcat(final, bn_parsenum(bn_replacenum(word, i)));
            ...
            final = strcat(final, bn_spellNumbers(word, i + 1, size));
        }
    }
    else {
        /* Bengali Word ? */
...
char *
bn_spellNumbers(short *word, int start, int end) {
    int i;
...
char *
bn_parseMiscellaneous(short *s, int size) {
    char *phoneticScript = (char *) malloc(100 * sizeof (char *));
    int i = 0;
    char gap[] = " G3000 ";
    char *englishAlph[26] ={"7", "9B", "s3", "d3", "3", "7f", "j3", "7ch", "1y", "j7", "k3", "7l",
            ...
            if (i >= 1 && word[i].type == 2 && word[i - 1].type == 1) {
                ...
                if (cvflag[i + 1]) //if (cvflag[i + 1] || strcmp (word[i + 1].letter, "1") >= 0)
                {
                    ...
                    while (!hf && itr++ < (no_of_halfs - 1))
                        if (!strcmp(bn_available_halfs[itr], t_half))
                            hf = 1;
                    if (hf) {
                ...
                if (i > 2 && word[i - 2].type == 2 && word[i - 3].type == 1
                    ...
                    strcat(lsyl, bn_getvowel(word[i - 2].letter));
                    strcat(lsyl, word[i - 1].letter);
                    strcat(lsyl, syllable);
                    strcpy(syllable, lsyl);
...
char *
bn_replacenum(unsigned short *s, int size) {
    int i, j;
    char *numchar;
    i = 0;
    j = 0;
