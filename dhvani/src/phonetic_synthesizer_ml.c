...
void ml_readfile ();
char *ml_process (short *word, int size);
int ml_replace (unsigned short *s, int size);
char *ml_replacenum (unsigned short *s, int size);
char *ml_parsenum (char *numstr);
char *ml_spellNumbers (short *word, int start, int end);
char *ml_parseMiscellaneous (short *s, int size);
...
char *
generate_phonetic_script_ml (short *word, int size)
{
  ...
  if ((word[0] >= 0x0D66 && word[0] <= 0x0D6F)
      ...
      if (i == size || i == size - 1)	//No decimal points
	{
	  if (size > 9)
	    ...
	    {			//construct the number string using ml_parsenum logic
	      //printf("reading numbers\n");
	      final =
		strcat (final, ml_parsenum (ml_replacenum (word, size)));
	    }
	}
      else
	{
...
char *
ml_spellNumbers (short *word, int start, int end)
{
  int i;
...
char *
ml_parseMiscellaneous (short *s, int size)
{
