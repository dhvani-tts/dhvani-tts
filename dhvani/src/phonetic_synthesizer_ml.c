...
char *available_halfs[] ={"ky", "kr", "kl", "kll", "kv", "ksh", "khy", "khr", "khl", "khv",
    "gy",
...
int
ml_replace(unsigned short *s, int size) {
    int i, j;
...
char *
generate_phonetic_script_ml(unsigned short *word, int size) {
    ...
    if ((word[0] >= 0x0D66 && word[0] <= 0x0D6F)
        ...
        if (i == size || i == size - 1) //No decimal points
        {
            if (size > 9) {
                ...
                final = strcat(final, ml_spellNumbers(word, 0, size));
            } else { //construct the number string using ml_parsenum logic
                //printf("reading numbers\n");
                final = strcat(final, ml_parsenum(ml_replacenum(word, size)));
            }
        } else {
            ...
            if (i > 9) {
                ...
                //Read each numbers seperately  
                //      printf("spell out numbers\n");
                final = strcat(final, ml_spellNumbers(word, 0, i));
                //      printf("spell out numbers-over\n");
            } else { //construct the number string using ml_parsenum logic
                //      printf("speak numbers%d\n",i);
                final = strcat(final, ml_parsenum(ml_replacenum(word, i)));
            ...
            final = strcat(final, ml_spellNumbers(word, i + 1, size));
        }
    }
    else {
        /* Malayalam Word ? */
...
char *
ml_spellNumbers(unsigned short *word, int start, int end) {
    int i;
    char *final = (char *) malloc(100 * sizeof (char *));
    final[0] = '\0';
    unsigned short *decimal = (short *) malloc(100 * sizeof (short *));
    for (i = start; i < end; i++) {
...
char *
ml_parseMiscellaneous(unsigned short *s, int size) {
            ...
            if (i >= 1 && word[i].type == 2 && word[i - 1].type == 1) {
                ...
                if (cvflag[i + 1]) //if (cvflag[i + 1] || strcmp (word[i + 1].letter, "1") >= 0)
                {
                    ...
                    while (!hf && itr++ < (no_of_halfs - 1))
                        if (!strcmp(available_halfs[itr], t_half))
                            hf = 1;
                    if (hf) {
                ...
                if (i > 2 && word[i - 2].type == 2 && word[i - 3].type == 1
                    ...
                    strcat(lsyl, ml_getvowel(word[i - 2].letter));
                    strcat(lsyl, word[i - 1].letter);
                    strcat(lsyl, syllable);
                    strcpy(syllable, lsyl);
...
char *
ml_replacenum(unsigned short *s, int size) {
    int i, j;
    char *numchar;
    i = 0;
    j = 0;
