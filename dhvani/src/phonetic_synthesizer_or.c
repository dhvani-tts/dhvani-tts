...
int
or_replace (unsigned short *s, int size)
{
  int i, j;
  ...
  while (i < size)
   ...
   /*--------------CONSONANTS-----------------*/
	...
	case 0x0B31:		/*R */
	  if (i + 2 <= size && s[i + 2] == 0x0B31 && s[i + 1] == 0x0B4D)
	    {
	      // handling rra-> tra    
	      word[j].letter = "t";
	      word[j].type = 1;
...
char *
generate_phonetic_script_or (short *word, int size)
{
	  ...
	  if (size > 9)
	    {
	      ...
	      final = strcat (final, or_spellNumbers (word, 0, size));
	    }
	  else
	    {			//construct the number string using or_parsenum logic
	      //printf("reading numbers\n");
	      final =
		strcat (final, or_parsenum (or_replacenum (word, size)));
	    }
	}
      else
	{
	  ...
	  if (i > 9)
	    {
	      ...
	      //Read each numbers seperately  
	      //      printf("spell out numbers\n");
	      final = strcat (final, or_spellNumbers (word, 0, i));
	      //      printf("spell out numbers-over\n");
	    }
	  else
	    {			//construct the number string using or_parsenum logic
	      //      printf("speak numbers%d\n",i);
	      final = strcat (final, or_parsenum (or_replacenum (word, i)));
...
//        printf("say dashamsham");
	  ...
	  final = strcat (final, or_spellNumbers (word, i + 1, size));
	}
    }
...
char *
or_spellNumbers (short *word, int start, int end)
{
  int i;
...
char *
or_parseMiscellaneous (short *s, int size)
{
	  ...
	  if (i >= 1 && word[i].type == 2 && word[i - 1].type == 1)
	      ...
	      if (cvflag[i + 1])	//if (cvflag[i + 1] || strcmp (word[i + 1].letter, "1") >= 0)
		{
		  ...
		  while (!hf && itr++ < (no_of_halfs - 1))
		    if (!strcmp (or_available_halfs[itr], t_half))
		      hf = 1;
		  if (hf)
		    {
	      ...
	      else if (i > 1 && word[i - 2].type == 0 && !hf)
		{
		  ...
		  strcat (lsyl, or_getvowel (word[i - 2].letter));
		  strcat (lsyl, word[i - 1].letter);
		  strcat (lsyl, syllable);
		  strcpy (syllable, lsyl);
...
char *
or_replacenum (unsigned short *s, int size)
{
  int i, j;
  char *numchar;
  i = 0;
