...
char *alph[] ={"0", "00", "none", "1", "2", "3", "4", "5", "6", "r5", "ll5", "none",
    "7",
...
char *ka_halfs[] ={"ky", "kr", "kl", "kll", "kv", "ksh", "khy", "khr", "khl", "khv",
    "gy",
...
readfile (FILE * fd)*/
char *
generate_phonetic_script_ka(short *word, int size) {
    int j = 0;
    char *final;
...
char *
ka_replacenum(unsigned short *s, int size) {
    int i, j;
    char *numchar;
    i = 0;
    j = 0;
...
iscon(char *symbol) {
    int i = 0;
    char *cons[] ={"k", "kh", "g", "gh", "ch", "chh", "j", "jh", "tt", "tth", "dd",
        "ddh",
...
int
ka_replace(unsigned short *s, int size) {
    int k, j = 0;
    for (k = 0; k < size; k++) {
    ...
    if (strcmp(word[i], "gn")) {
        ...
        if (strlen(word) < (i + 1))
            if (strcmp(word[i + 1], "k") ||
                    ...
                    strcmp(word[i + 1], "t") ||
                    strcmp(word[i + 1], "th") ||
                    strcmp(word[i + 1], "d") || strcmp(word[i + 1], "dh")) {
                word1[j].type = 1;
                word1[j++].letter = "n";
            }
...
ka_breakit(int last) {
    ...
    for (i = 0; i < last + 5; i++) {
        if (i + 1 < last && word1[i].type == 1 && word1[i + 1].type == 0) {
            cvflag[i] = 1;
            cvflag[i + 1] = 2;
            i++;
        } else {
    ...
    for (i = last - 1; i >= 0; i--) {
        ...
        if (cvflag[i] == 2) {
            ...
            if ((i >= 1) && (word1[i].type == 2) && (word1[i - 1].type == 1)) {
                /* there is a cH */

                if (cvflag[i + 1]
                        || *(syllable + strlen(word1[i + 1].letter) + 1) == '1') {
                    /* if cv or c1 follows current conH */
                    itr = 0;
                    strcpy(t_half, word1[i - 1].letter);
                    strcat(t_half, word1[i + 1].letter);
                    while (!hf && itr++ < (no_of_halfs - 1))
                        if (!strcmp(ka_halfs[itr], t_half))
                            hf = 1;
                    if (hf) {
                        strcpy(lsyl, word1[i - 1].letter);
                        strcat(lsyl, "H");
                        strcat(lsyl, ++syllable);
                        strcpy(syllable, lsyl);
                ...
                if (i > 2 && word1[i - 2].type == 2
                    ...
                    dcr = 1;
                } else if (i > 1 && word1[i - 2].type == 1 && !hf) {
                    ...
                    dcr = 2;
                } else if (i > 1 && word1[i - 2].type == 0 && !hf) {
                    /* there is a v behind cH giving vc */
                    if (*word1[i - 2].letter == 'c') {
                        strcpy(lsyl, word1[i - 3].letter);
                        dcr = 1;
                    } else {
                        strcpy(lsyl, "");
                    }
                    strcat(lsyl, ka_getvowel(word1[i - 2].letter));
                    strcat(lsyl, word1[i - 1].letter);
                    strcat(lsyl, syllable);
                    strcpy(syllable, lsyl);
...
char *
ka_parsenum(char *numstr) {

    char *singles[] ={"s11n n7", "11n d5", "7 r1 dd5", "m6 r5", "n2l k5", "10 d5",
        "2 r5",
        "8 ll5", "7nna tt5", "11m bh1t t5"
    };
    char *teens[] ={"h1t t5", "h1n n11n d5", "h1n n7 r1 dd5", "h1 d3 m6 r5",
        "h1 d3 n2l k5",
        "h1 d3 n10 d5", "h1 d3 n2 r5", "h1 d3 n8 ll5", "h1 d3 n7nna tt5",
        "h1t t11m bh1t t5"
    };
    char *tens[] ={" ", " ", "3p p1t t1", "m6 v1t t1", "n1 l1 v1t t1", "10 v1t t1",
        "1 r1 v1t t1", "7p p1t t1", "7m bh1t t1", "t11m bh1t t1"
    };
    char *hundreds[] ={"", "n6 r1", "3n n6 r1", "m5n n6 r1", "n2 n6 r1", "10 n6 r1",
