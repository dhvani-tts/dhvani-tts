...
int ps_replace(unsigned short *s, char **s1, unsigned char *template,
        int size);
...
int
ps_replace(unsigned short *s, char **s1, unsigned char *template, int size)
{
        int i = 0, j = 0;
        while (i < size) {
                switch (s[i]) {
...
char *
generate_phonetic_script_ps(unsigned short *word, int size)
{
        ...
        if (word[0] >= 0x0966 && word[0] <= 0x096F) { /* Number ? */
                ...
                if (special && (size > special)) { /* prefix, ie 'sub-word' found */
                        ...
                        while (special && (start <= size)) {
                                ...
                                ps_parseword(subarr1, subtemplate1, special + 1, phon, final);
