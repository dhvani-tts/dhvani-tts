...
int oggenc(char* inputfile, char* outputfile, dhvani_ogg_parameters *ogg_parameters) {
    ...
    readbuffer[0] = '\0';
    for (i = 0, founddata = 0; i < 30 && !feof(input_fd) && !ferror(input_fd); i++) {
    ...
    ret = vorbis_encode_init_vbr(&vi, ogg_parameters->channels, ogg_parameters->sampling_rate / ogg_parameters->channels, ogg_parameters->quality);
