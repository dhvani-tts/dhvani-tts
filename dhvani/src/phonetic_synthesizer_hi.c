...
char *
generate_phonetic_script_hi (short *word, int size)
{
  ...
  char *outstr = (char *) malloc (5000 * sizeof (char *));


  if (word[0] >= 0x0966 && word[0] <= 0x096F)
    {				/* Number ? */
    ...
    {				/* Word ? */
      ...
      if (special && (size > special))
	{			/* prefix, ie 'sub-word' found */
	      ...
	      char *final)
{
  ...
  testhalf = (char *) malloc (10 * sizeof (char *));
  ...
  while (i + 1 < arrsz)
	...
	{			/* CV */
      ...
      else if ((template[i] == 2) && (template[i + 1] == 0))
	{			/* ChV */
	  cvmarks[i - 1] = 1;
	  i += 2;
	  j++;
  ...
  if ((template[arrsz - 1] == 1) && (arrsz > 1))
    {				/* if ends in C,make it Ch */
      arr[arrsz] = "H";
      template[arrsz] = 2;
      arrsz++;
      cend = 1;
  ...
  for (i = arrsz - 1; i >= 0; i--)
	      ...
	      if (i > 1)
		{
		  if (!cvmarks[i - 2])
		    ...
		    {		/* CVC */
		      ...
		      if ((arr[i - 1] == "1") && (arr[i] == "h"))
			{	/* Special case - o/p C7C */
			  phon[k++] = arr[i];
			  phon[k++] = "7";
			  phon[k++] = arr[i - 2];
			  i -= 2;
	...
	{			/* Consonant-Halant */
	  ...
	  if (((i == 1) && (template[i + 2] != 2)) || (template[i - 2] == 2))
	    {			/*Another Consonant-Halant */
	      if (((i < arrsz - 1) && (cvmarks[i + 1])) || (i == 1))
		{		/* Half-sound?? */
		  ...
		  if (hi_ishalf (testhalf))
		    ...
		    {		/* no,we don't have the 'half' sound,so output 0C */
		...
		{		/* need not check for 'half' sound, just output 0C */
	    ...
	    {			/* Just a Ch, so output 0C */
	      phon[k++] = arr[i - 1];
	      phon[k++] = "0";
	      i--;
	    }

	  else if (template[i - 2] == 1)
	    {			/* A consonant, so output C1C */
	      phon[k++] = arr[i - 1];
	      phon[k++] = "1";
	      phon[k++] = arr[i - 2];
	      i -= 2;
	  ...
	  else if (template[i - 2] == 0)
	    {			/* Vowel */
	      if (i > 1)
		{
		  if ((i - 3 < 0) || (!cvmarks[i - 3]))
		    {		/* Not CVC ? Output VC */
		      phon[k++] = arr[i - 1];
		      phon[k++] = arr[i - 2];
		      i -= 2;
	...
	{			/* Vowel */
	  if (i == 0)
	    {			/* Lone vowel, just output V */
	      phon[k++] = arr[i];
	    }

	  else if (cvmarks[i - 1])
	    {			/* Part of a CV */
	      if ((arr[i] == "13") && (arr[i + 1] == "v"))
		{		/* special cases */
		  phon[k++] = "14";
		  phon[k++] = arr[i - 1];
		  i--;
		}
	      else if ((arr[i] == "9") && (arr[i + 1] == "y")
		       && (i < arrsz - 1))
		{
		  phon[k++] = "10";
		  phon[k++] = arr[i - 1];
	  ...
	  else if ((template[i - 1] == 2) && (cvmarks[i - 2]))
	    {			/* Part of a ChV */
	      phon[k++] = arr[i];
	      phon[k++] = arr[i - 2];
	      i -= 2;
...
hi_ishalf (char *testhalf)
{
  ...
  while (strcmp (halves[i], "end"))
    {				/* Traverse the list of 'half' sounds  */
...
hi_parsenum (int *arr, int arrsz, char *final)
{
  ...
  if (arrsz == 1)
    ...
    {				/* between hundred & thousand - special handling reqd */
      strcat (final, units[arr[i]]);
      strcat (final, " G3000 ");
      i++;
      /* 'this many' hundreds */
      ...
      if (i)
	{
      ...
      for (; i < arrsz - 3; i += 2)
	{			/* repeat till less than one thousand */
	  /* next two digits */
	  strcat (final, uptohundred[arr[i]][arr[i + 1]]);
	  strcat (final, " G3000 ");
	  j++;
