...
if( (character==0x0BBE || character==0x0BBF || character==0x0BC0 || character==0x0BC1 || character==0x0BC2 || character==0x0BC6 || character==0x0BC7||character==0x0BC8||character==0x0BCA||character==0x0BCB||character==0x0BCC)){
return 1;
}
return 0;
}
int isIntervocalical(unsigned short string[], int size, int position){
if( position-1 > 0 && position+1  <size && ((  isVowel(string[position-1]) && isVowel(string[position+1])) || (string[position-1]!=0x0BCD && string[position+1]!=0x0BCD ) )){
return 1;
}
return 0;
}
  ...
  if ((word[0] >= 0x0D66 && word[0] <= 0x0D6F)
      ...
      if (i == size || i == size - 1)	//No decimal points
	{
	  if (size > 9)
	    ...
	    {			//construct the number string using ta_parsenum logic
	      //printf("reading numbers\n");
	      final =
		strcat (final, ta_parsenum (ta_replacenum (word, size)));
	    }
	}
      else
	{
...
ta_parseMiscellaneous (short *s, int size)
{
  ...
  char gap[] = " G3000 ";
  char *englishAlph[26] =
    { "7", "b3", "s3", "d3", "3", "7f", "j3", "7ch", "1y", "j7", "k3", "7l",
...
ta_parseword (int last)
{
  ...
  for (i = 0; i < last + 5; i++)
    {
      if (i + 1 < last && word[i].type == 1 && word[i + 1].type == 0)
	{			//A CV
	  cvflag[i] = 1;
	  cvflag[i + 1] = 2;
	  i++;
