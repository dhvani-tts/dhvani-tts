...
char *available_halfs_ta[] = {"ky", "kr", "kl", "kll", "kv", "ksh", "khy", "khr", "khl", "khv",
    "gy",
...
int
ta_replace(unsigned short *s, int size) {
    int i, j;
    ...
    while (i < size) {
        switch (s[i]) {
            ...
            case 0x0B95: /*k */
                if (i - 2 >= 0 && s[i - 2] == 0x0B99 && s[i - 1] == 0x0BCD) {
            ...
            case 0x0B9A: /*ch */
                if (i - 2 >= 0 && s[i - 2] == 0x0B9E && s[i - 1] == 0x0BCD) {
...
int
isVowel(unsigned short character) {
    if ((character == 0x0BBE || character == 0x0BBF || character == 0x0BC0
            || character == 0x0BC1 || character == 0x0BC2 || character == 0x0BC6
            || character == 0x0BC7 || character == 0x0BC8 || character == 0x0BCA
...
int
isIntervocalical(unsigned short string[], int size, int position) {
...
char *
generate_phonetic_script_ta(short *word, int size) {
    ...
    if ((word[0] >= 0x0D66 && word[0] <= 0x0D6F)
        ...
        if (i == size || i == size - 1) //No decimal points
        {
            if (size > 9) {
                ...
                final = strcat(final, ta_spellNumbers(word, 0, size));
            } else { //construct the number string using ta_parsenum logic
                //printf("reading numbers\n");
                final = strcat(final, ta_parsenum(ta_replacenum(word, size)));
            }
        } else {
            ...
            if (i > 9) {
                ...
                //Read each numbers seperately  
                //      printf("spell out numbers\n");
                final = strcat(final, ta_spellNumbers(word, 0, i));
                //      printf("spell out numbers-over\n");
            } else { //construct the number string using ta_parsenum logic
                //      printf("speak numbers%d\n",i);
                final = strcat(final, ta_parsenum(ta_replacenum(word, i)));
            ...
            final = strcat(final, ta_spellNumbers(word, i + 1, size));
        }
    } else {
        /* Tamil Word ? */
...
char *
ta_spellNumbers(short *word, int start, int end) {
    int i;
...
char *
ta_parseMiscellaneous(short *s, int size) {
    char *phoneticScript = (char *) malloc(100 * sizeof (char *));
    int i = 0;
    char gap[] = " G3000 ";
    char *englishAlph[26] = {"7", "b3", "s3", "d3", "3", "7f", "j3", "7ch", "1y", "j7", "k3", "7l",
...
ta_parseword(int last) {
    ...
    for (i = last - 1; i >= 0; i--) { //right to left
        ...
        if (cvflag[i] == 2) {
            ...
            if (i >= 1 && word[i].type == 2 && word[i - 1].type == 1) {
                /* there is a cH */
                ...
                if (cvflag[i + 1]) //if (cvflag[i + 1] || strcmp (word[i + 1].letter, "1") >= 0)
                {
                    ...
                    while (!hf && itr++ < (no_of_halfs - 1))
                        if (!strcmp(available_halfs_ta[itr], t_half))
                            hf = 1;
                    if (hf) {

                        strcpy(lsyl, word[i - 1].letter);
                        strcat(lsyl, "H");
                        strcat(lsyl, ++syllable);
                        strcpy(syllable, lsyl);
                ...
                if (i > 2 && word[i - 2].type == 2 && word[i - 3].type == 1
                        && !(cvflag[i - 2] == 2 && cvflag[i - 3] == 1) && !hf) {
                    ...
                    dcr = 1;
                } else if (i > 1 && word[i - 2].type == 1 && !hf) {
                    ...
                    dcr = 2;
                } else if (i > 1 && word[i - 2].type == 0 && !hf) {
                    /* there is a v behind cH giving vc */
                    if (*word[i - 2].letter == 'c') {
                        strcpy(lsyl, word[i - 3].letter);
                        dcr = 1;
                    } else {
                        strcpy(lsyl, "");
                    }
                    strcat(lsyl, ta_getvowel(word[i - 2].letter));
                    strcat(lsyl, word[i - 1].letter);
                    strcat(lsyl, syllable);
                    strcpy(syllable, lsyl);
                    ...
                    dcr = 2 + dcr;
                } else if (i == 1 && word[i].type == 2 && word[i - 1].type == 1 && !hf) {
...
char *
ta_parsenum(char *numstr) {

    char *singles[] = {"p6j y1m", "11n n15", "r1n dd15", "m6n n15", "n2l l15", "1n ch15",
        "2r r15",
        "8 zh15", "7t tt15", "11m p1 t15"
    };
    char *teens[] = {"p1t t15", "p1t t3n 11n n15", "p1n d1 r1n dd15", "p1 t3 m6 n15",
        "p1 t3 n2 l15", "p1 t3 n1n ch15", "p1 t3 n2r r15", "p1 t3 n8 zh15",
        "p1 t3 n7t t15",
        "p1 t11m p1 t15"
    };
    char *tens[] = {" ", " ", "3 r5 p1 t", "m5p p1 t", "n1l p1 t", "1m p1 t",
        "1 r5 p1 t", "7 zh5  p1 t", "7m p1 t", "t11n n6 r"
    };
    char *hundreds[] = {"", "n6 r1", "3 r5n n6 r1", "m5n n6 r1", "n2 n6 r1", "10n n6 r1",
...
char *
ta_replacenum(unsigned short *s, int size) {
    int i, j;
    char *numchar;
    i = 0;
    j = 0;
